...
void InterpolateLikelihood::readNewDataBaseFile(std::string filename) {
  ...
  if (infile.is_open()) {
    ...
    int phiNum = 30;
    int betaNum = 34;
    int Comibinations = xNum*yNum*thetaNum*phiNum*betaNum;
    int pixelNum = 1156;// (32+1 overflow + 1 underflow)(32+1 overflow + 1 underflow)

    measuredBatch currentBatch;
    std::vector<double> means;
    
    betaList.reserve(Comibinations);
    xList.reserve(Comibinations);
    yList.reserve(Comibinations);
    thetaList.reserve(Comibinations);
    phiList.reserve(Comibinations);
    measurements.reserve(Comibinations);
    means.reserve(pixelNum);
...
std::vector<std::vector<int>> InterpolateLikelihood::getAllPoints(std::vector<int> v1, std::vector<int> v2) {
  std::vector<std::vector<int>> allPoints;
  allPoints.push_back(v2);
...
std::vector<int> InterpolateLikelihood::returnSwap(std::vector<int> v1, std::vector<int> v2, std::vector<int> permSwp) {
  for (int i = 0; i < v1.size(); i++) {
    if (permSwp.at(i) == 1) {
      v1.at(i) = v2.at(i);
    }
...
void InterpolateLikelihood::removeDuplicates(std::vector<std::vector<int>> &vec) {
  std::sort(vec.begin(), vec.end());
  vec.erase(std::unique(vec.begin(), vec.end()), vec.end());
}
std::vector<std::vector<double>> InterpolateLikelihood::getSurroundingPointsIndex(std::vector<double> inputVariables){
  	
	int xSteps = TMath::Floor((inputVariables[0]-xList[0])/xDelta);
	int ySteps = TMath::Floor((inputVariables[1]-yList[0])/yDelta);
	int thetaSteps = TMath::Floor(inputVariables[2]/thetaDelta);
	int phiSteps = TMath::Floor(inputVariables[3]/phiDelta);
	int betaSteps = 0;
	if (inputVariables[4]>0.999){
		betaSteps = 20+TMath::Floor((inputVariables[4]-betaChangePoint)/betaDelta_2);
	}
	else { betaSteps = TMath::Floor((inputVariables[4]-betaList[0])/betaDelta_1);}

	std::cout << xSteps << "   ";
	std::cout << ySteps << "   ";
	std::cout << thetaSteps << "   ";
	std::cout << phiSteps << "   ";
	std::cout << betaSteps << std::endl;
	
	int index = calcIndexEquation(xSteps,ySteps,thetaSteps,phiSteps,betaSteps);

	std::cout << index << std::endl;
	std::cout << "TEST ONE: " << std::endl;
	std::cout << inputVariables[0] << "   ";
	std::cout << inputVariables[1] << "   ";
	std::cout << inputVariables[2] << "   ";
	std::cout << inputVariables[3] << "   ";
	std::cout << inputVariables[4] << std::endl;

	std::cout << measurements[index].x << "   ";
	std::cout << measurements[index].y << "   ";
	std::cout << measurements[index].theta << "   ";
	std::cout << measurements[index].phi << "   ";
	std::cout << measurements[index].beta << std::endl;
	

	std::vector<double> m = { measurements[index].x, measurements[index].y, measurements[index].theta, measurements[index].phi, measurements[index].beta};
	std::vector<int> upperSteps={xSteps,ySteps,thetaSteps,phiSteps,betaSteps};
	std::vector<int> lowerSteps={xSteps,ySteps,thetaSteps,phiSteps,betaSteps};
	std::vector<int> MaxSteps = {xNum,yNum,thetaNum,phiNum,betaNum_1+betaNum_2};
	std::cout << "Upper:      Lower:" << std::endl; 
	for (int i = 0; i < inputVariables.size(); i++)
	{
		if (inputVariables[i] != m[i]){
			upperSteps[i] += 1;
			if (upperSteps[i] > MaxSteps[i]){upperSteps[i] = MaxSteps[i];} 
			if (lowerSteps[i] < 0) {lowerSteps[i] = 0;}
		}
		std::cout << upperSteps[i] << "   " << lowerSteps[i] << std::endl;
		
	}
        std::vector<std::vector<int>> vAllSteps = getAllPoints(upperSteps,lowerSteps);
	std::vector<std::vector<double>> variablesList;
	for (int i = 0; i < vAllSteps.size(); i++)
	{
		int t_index = calcIndexEquation(vAllSteps[i][0],
				vAllSteps[i][1],
				vAllSteps[i][2],
				vAllSteps[i][3],
				vAllSteps[i][4]);

		std::vector<double> m = { measurements[t_index].x,
					  measurements[t_index].y, 
					  measurements[t_index].theta, 
					  measurements[t_index].phi, 
					  measurements[t_index].beta};
		variablesList.push_back(m);

		std::cout << i << "   ";
		std::cout << measurements[t_index].x << "   ";
		std::cout << measurements[t_index].y << "   ";
		std::cout << measurements[t_index].theta << "   ";
		std::cout << measurements[t_index].phi << "   ";
		std::cout << measurements[t_index].beta << std::endl;
	}
...
std::vector<std::vector<double>> InterpolateLikelihood::getGaussianValuesAlongAxis(std::vector<std::vector<double>> vAll, std::vector<double> v, std::vector<bool> vSwitch) {
  ...
  //std::cout << "ALL VALUES: " << vAll.size() << std::endl;
  std::vector<std::vector<double>> meansOfInterpolatedSpline;
  meansOfInterpolatedSpline.reserve(vAll.size());
  for (int i = 0; i < vAll.size(); i++) {
    std::vector<int> meanIndex;
    std::vector<double> variableList;
    std::vector<std::vector<double>> tempGaussianParameters;
    getListOfVariableIndex(vSwitch, vAll[i], meanIndex, variableList);
    //std::cout << "MEAN INDEX:" << meanIndex.size() << std::endl;
    //std::cout << "VARIABLE INDEX:" << variableList.size() << std::endl;
    tempGaussianParameters = getGaussian(variableList, meanIndex, "test");
    //std::cout << "SPLINE INDEX:" << tempSplines.size() << std::endl;
    std::vector<double> tempMeans;
    tempMeans.reserve(tempGaussianParameters.size());
    //std::cout << "EVAL SPLINE: " << std::endl;
    for (int k = 0; k < tempGaussianParameters.size(); k++) {
      double p0 = tempGaussianParameters[k][0];
      double p1 = tempGaussianParameters[k][1];
      double p2 = tempGaussianParameters[k][2];
...
std::vector<int> InterpolateLikelihood::getReducedVariableList(double v, std::vector<double> variableList)
{
	int length = 4;
	std::vector<int> shortList;
	shortList.reserve(length);
	double minIndex = 0;
	double nearestParameter = 1E6;
	//Finds the index of the closest variable
	for (int i = 0; i < variableList.size(); i++)
	{
		double delta = std::fabs(variableList[i]-v);
		if (delta < nearestParameter)
		{
			minIndex = i;
		}
	}
	std::cout << "MIN INDEX:" << minIndex << std::endl;
	//Finds the stop range while considering the 
	//boundary conditions
	double stopRange;
	if (minIndex == variableList.size()-1)
	{
		stopRange = minIndex;	
	}
	else if (minIndex == variableList.size()-2)
	{
		stopRange = minIndex+1;
	}
	else if (minIndex == 0)
	{
		stopRange = 0+length;
	...
	else if (minIndex ==1)
	{
		...
		stopRange = minIndex + 2;
	}
	
	for (int i = stopRange - length; i <= stopRange; i++)
	{
...
std::vector<std::vector<double>> InterpolateLikelihood::getPolynomialValuesAlongAxis(std::vector<std::vector<double>> vAll, std::vector<double> v, std::vector<bool> vSwitch) {
  ...
  std::cout << "ALL VALUES: " << vAll.size() << std::endl;
  std::vector<std::vector<double>> meansOfInterpolatedPoly;
  meansOfInterpolatedPoly.reserve(vAll.size());
  //Loops through all surronding points
  for (int i = 0; i < vAll.size(); i++) {
    std::cout << i << std::endl;
    std::cout << vAll[i].size() << std::endl;
    std::vector<int> meanIndex; //used for selecting means from measurement struct
    std::vector<double> variableList; // used to select variable list
    std::vector<std::vector<double>> tempPolynomialParameters; // polynomial parameters used in interpolation
    //getListOfVariableIndex(vSwitch, vAll[i], meanIndex, variableList); //sets MeanIndex and VariableList to the value required
    getRangeOfIndex(vSwitch, vAll[i], meanIndex, variableList); //sets MeanIndex and VariableList to the value required

    std::cout << "MEAN INDEX:" << meanIndex.size() << std::endl;
    std::cout << "VARIABLE INDEX:" << variableList.size() << std::endl;
    
    std::vector<int> selectedIndexes = getReducedVariableList(v[choiceVariable],variableList);
    std::cout << selectedIndexes.size() << std::endl;
    tempPolynomialParameters = getPolynomialFit(selectedIndexes, variableList, meanIndex, "test");
    std::cout << "SPLINE INDEX:" << tempPolynomialParameters.size() << std::endl;
    std::vector<double> tempMeans;
    tempMeans.reserve(tempPolynomialParameters.size());
    //std::cout << "EVAL SPLINE: " << std::endl;
    for (int k = 0; k < tempPolynomialParameters.size()-1; k++) {
      double p0 = tempPolynomialParameters[k][0];
      double p1 = tempPolynomialParameters[k][1];
      double p2 = tempPolynomialParameters[k][2];
      double tMean = p0 + p1*v.at(choiceVariable) + p2*v.at(choiceVariable)*v.at(choiceVariable);
      std::cout << "PIXEL: " << k << ":   "
                << "v:" << v.at(choiceVariable) << "  " << p0 << "  " << p1 << "  " << p2 << "  " << tMean << std::endl;
      if (tMean < 0) {
        tMean = 1E-20;
      }
      tempMeans.push_back(tMean);
    ...
    meansOfInterpolatedPoly.push_back(tempMeans);
  }
  return meansOfInterpolatedPoly;
}
...
std::vector<std::vector<int>> InterpolateLikelihood::removeVariable(std::vector<std::vector<int>> vAll, std::vector<bool> vSwitch) {
  for (int i = 0; i < vAll.size(); i++) {
    for (int j = 0; j < vAll[i].size(); j++) {
      if (vSwitch[j]) {
        vAll[i][j] = 0;
...
std::vector<double> InterpolateLikelihood::interpolateMeansLinearlyWithPolynomialApproximation(std::vector<double> variables, std::vector<bool> vSwitch) {
  /*
  std ::cout << "##################" << std::endl;
  ...
  std::vector<std::vector<double>> vAll = getAllPoints(vUpp, vLow);*/
  std::vector<std::vector<double>> vAll = getSurroundingPointsIndex(variables);

  std::vector<double> vDiff = getDifferenceBetween2Vectors(vAll.back(), vAll.front());
  vAll = removeVariable(vAll, vSwitch);
  ...
  //checkVector(vAll[i]);
  //}
  //std::vector<int> indexList = getSurroundingPointsIndex(variables,vSwitch);
  std::vector<std::vector<double>> meansAlongAxis = getPolynomialValuesAlongAxis(vAll, variables, vSwitch);
  std::vector<double> means(measurements.at(0).mean[0].size(), 0.0);
  //std::cout << "MEANS ALONG AXIS" << std::endl;
  //for (int i = 0; i < meansAlongAxis.size(); i++) {
  //std::cout << i << "   ";
...
int InterpolateLikelihood::calcIndexEquation(int xSteps, int ySteps, int thetaSteps, int phiSteps, int betaSteps)
{
	int betaNum = betaNum_1+betaNum_2;
	return (betaNum*phiNum*thetaNum*yNum)*xSteps +
	       (betaNum*phiNum*thetaNum)*ySteps +
               (betaNum*phiNum)*thetaSteps +
	       (betaNum)*phiSteps+
	       (betaSteps);
}

void InterpolateLikelihood::getRangeOfIndex(std::vector<bool> variableSwitch,
					    ...
					    std::vector<double> &listOfVariables)
{
	...
	std::vector<int> steps = {xSteps,ySteps,thetaSteps,phiSteps,betaSteps};

	int num = 0;
	if (variableSwitch[0] == true) {num = xNum;}
	else if (variableSwitch[1] == true) {num = yNum;}
	else if (variableSwitch[2] == true) {num = thetaNum;}
	else if (variableSwitch[3] == true) {num = phiNum;}
	else if (variableSwitch[4] == true) {num = betaNum_1+betaNum_2;}
	for (int i = 0; i < num; i++){
		for (int v = 0; v < variableSwitch.size(); v++)
		{
			if (variableSwitch[v]) {steps[v] = i;}	
		}
		meanIndexList.push_back(calcIndexEquation(steps[0], steps[1], steps[2], steps[3], steps[4]));
		if (variableSwitch[0] == true) {listOfVariables.push_back(measurements[meanIndexList.back()].x);}
		else if (variableSwitch[1] == true) {listOfVariables.push_back(measurements[meanIndexList.back()].y);}
		else if (variableSwitch[2] == true) {listOfVariables.push_back(measurements[meanIndexList.back()].theta);}
		else if (variableSwitch[3] == true) {listOfVariables.push_back(measurements[meanIndexList.back()].phi);}
		else if (variableSwitch[4] == true) {listOfVariables.push_back(measurements[meanIndexList.back()].beta);}
...
std::vector<std::vector<double>> InterpolateLikelihood::getGaussian(std::vector<double> independent, std::vector<int> dependent, std::string fileName = "") {
  //std::string pathName;
  //pathName = "./Images/";

  std::vector<std::shared_ptr<TGraph>> gTempPlot;
  std::vector<std::vector<double>> meanGaussian;
  meanGaussian.reserve(measurements.at(0).mean.at(0).size());
  int colorCount = 0;
  ...
  for (int pixel = 0; pixel < measurements.at(0).mean.at(0).size(); pixel++) {
    ...
    TFitResultPtr temp = gTemp->Fit("gaus", "QSNC");
    std::vector<double> pixelParameters;
    pixelParameters.reserve(3);
    pixelParameters.push_back(temp->Parameter(0));
    pixelParameters.push_back(temp->Parameter(1));
    pixelParameters.push_back(temp->Parameter(2));
    meanGaussian.push_back(pixelParameters);
  }
  return meanGaussian;
}

std::vector<std::vector<double>> InterpolateLikelihood::getPolynomialFit(std::vector<int> selectedIndexes,std::vector<double> independent, std::vector<int> dependent, std::string fileName = "") {
  //std::string pathName;
  //pathName = "./Images/";

  std::vector<std::shared_ptr<TGraph>> gTempPlot;
  std::vector<std::vector<double>> meanPolynomial;
  meanPolynomial.reserve(measurements.at(0).mean.at(0).size());
  int colorCount = 0;
  ...
  for (int pixel = 0; pixel < measurements.at(0).mean.at(0).size(); pixel++) {
    ...
    for (int i = 0; i < selectedIndexes.size(); i++) {
      std::cout << independent[selectedIndexes[i]] << "   " <<  dependent[selectedIndexes[i]] << std::endl;
      gTemp->SetPoint(i, independent[selectedIndexes[i]], measurements[dependent[selectedIndexes[i]]].mean[0][pixel]);
    }
    TFitResultPtr temp = gTemp->Fit("pol2", "QSNC");
    std::vector<double> pixelParameters;
    pixelParameters.reserve(3);
    pixelParameters.push_back(temp->Parameter(0));
    pixelParameters.push_back(temp->Parameter(1));
    pixelParameters.push_back(temp->Parameter(2));
    meanPolynomial.push_back(pixelParameters);
  }
...
std::vector<double> InterpolateLikelihood::interpolationSwitch(std::vector<double> variable,
  ...
  int interpolateMeansLinearlyWithPolynomial = 3;
  double xSteps = 0;
  double ySteps = 0;
  double phiSteps = 0;
  ...
  } else if (intSwitch == interpolateMeansLinearlyWithPolynomial){
    pixelMap = this->interpolateMeansLinearlyWithPolynomialApproximation(variable, vSwitch);
  }
  else {
    return pixelMap;
  }
